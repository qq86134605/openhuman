---
name: "source-command-ship-and-babysit"
...
# source-command-ship-and-babysit

Use this skill when the user asks to run the migrated source command `ship-and-babysit`.
...
3. **Pre-push hook policy** (per `AGENTS.md`): if a pre-push hook fails on something unrelated to your changes (pre-existing breakage on `main` in code you didn't touch), push with `--no-verify` and call it out in the PR body. If the hook fails on your own changes, fix and re-push. Don't ask — just do the right thing and tell the user what you did.
